...
class ManejadorDatosNYC:
    ...
    Esta clase encapsula toda la lógica para obtener el dataset de taxis de NYC,
    desde la descarga y descompresión hasta la carga en un DataFrame de pandas.
    Está diseñada para ser eficiente, robusta y fácil de usar.
    """
    # URL pública y confiable para el dataset de Enero 2021.
    DATA_URL = "https://github.com/DataTalksClub/nyc-tlc-data/releases/download/yellow/yellow_tripdata_2021-01.csv.gz"

    # Construcción de la ruta de datos de forma dinámica para que funcione en cualquier sistema.
    DATA_DIR = os.path.join(os.getcwd(), "ejercicios_bigdata", "datos")

    def __init__(self):
        """
        Constructor de la clase.
        Define las rutas completas a los archivos con los que trabajará la clase.
        """
    ...
    def _asegurar_directorio(self):
        """
        Método auxiliar privado para garantizar que el directorio de destino exista.
    ...
    def descargar_y_descomprimir(self):
        ...
        Este método es idempotente: si el archivo final ya existe, no realiza ninguna acción.
        Maneja errores de red y de sistema de archivos de forma segura.
        """
        self._asegurar_directorio()

        if os.path.exists(self.ruta_csv):
            print(f"✅ El archivo ya existe en: {self.ruta_csv}")
            return

        print(f"⬇️ Descargando datos comprimidos desde: {self.DATA_URL}")
        ...
        try:
            ...
            print(f"✅ Archivo descomprimido listo: {self.ruta_csv}")
    ...
    def cargar_datos(self) -> pd.DataFrame:
        ...
        if os.path.exists(self.ruta_csv):
            ...
            print("❌ No se pudieron cargar los datos. La descarga pudo haber fallado.")
            return pd.DataFrame()


class AnalizadorDatosNYC:
    """
    Clase para realizar análisis sobre el DataFrame de taxis de NYC.
    """
    def __init__(self, dataframe: pd.DataFrame):
        self.df = dataframe
        # Filtrar valores atípicos para una mejor visualización de los gráficos
    ...
    def mostrar_pasajeros_frecuentes(self):
        ...
        print(f"\nEl número de pasajeros más frecuente es {most_frequent_passenger} con {most_frequent_count} viajes.")
        ...
        if not passenger_counts_valid.empty:
            ...
            print(f"El número de pasajeros menos frecuente es {least_frequent_passenger} con {least_frequent_count} viajes.")
        else:
            print("No hay recuentos de pasajeros válidos para determinar el menos frecuente.")
    ...
    def grafico_distribucion_distancias(self):
        """Gráfico 1: Muestra la distribución de las distancias de viaje."""
        print("Generando Gráfico 1: Distribución de distancias...")
        plt.figure(figsize=(10, 6))
        sns.histplot(self.df_filtrado['trip_distance'], bins=50, kde=True)
        plt.title('Gráfico 1: Distribución de Distancias de Viaje')
    ...
    def grafico_distribucion_tarifas(self):
        """Gráfico 2: Muestra la distribución de las tarifas de viaje."""
        print("Generando Gráfico 2: Distribución de tarifas...")
        plt.figure(figsize=(10, 6))
        sns.histplot(self.df_filtrado['fare_amount'], bins=50, kde=True)
        plt.title('Gráfico 2: Distribución de Tarifas de Viaje')
        plt.xlabel('Tarifa ($)')
        plt.ylabel('Frecuencia')
        plt.grid(True)
        plt.show()
    ...
    def grafico_pasajeros_por_viaje(self):
        """Gráfico 3: Muestra un gráfico de barras del número de pasajeros por viaje."""
        print("Generando Gráfico 3: Pasajeros por viaje...")
        plt.figure(figsize=(10, 6))
        sns.countplot(x='passenger_count', data=self.df)
        plt.title('Gráfico 3: Conteo de Pasajeros por Viaje')
    ...
    def mostrar_menu_graficos(self):
        """Muestra un submenú para la visualización de gráficos."""
        while True:
            print("\n--- Menú de Gráficos ---")
            print("1. Gráfico 1: Distribución de distancias")
            print("2. Gráfico 2: Distribución de tarifas")
            print("3. Gráfico 3: Pasajeros por viaje")
            print("0. Volver al menú principal")
            opcion = input("Selecciona un gráfico para ver: ")
...
if __name__ == "__main__":
    ...
    if not df_taxis.empty:
        print("\n--- Datos cargados exitosamente. Iniciando análisis interactivo. ---")
        analizador = AnalizadorDatosNYC(df_taxis)
        analizador.mostrar_menu()
    else:
        print("\n--- No se pudo continuar con el análisis debido a un error en la carga de datos. ---")
